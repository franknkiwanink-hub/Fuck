...
type OutcomeState = "accept" | "reject" | "pending";
...
export default function DealPopup({
  ...
  // Ports _mpRenderOutcome's three theme branches — copy/timer differ per
  // state, same strings as the original.
  function renderOutcomeContent(state: OutcomeState) {
    if (state === "accept") {
      return {
        theme: "theme-accept",
        title: "Offer accepted!",
        sub: "The seller's agent approved your deal. Head to your inbox to coordinate next steps.",
        showTimer: false,
      };
    }
    if (state === "reject") {
      return {
        ...
        sub: "The seller's agent declined this offer. You can send a new deal with a different offer anytime.",
        showTimer: false,
      };
    }
    return {
      theme: "theme-pending",
      title: "Offer pending",
      sub: "The seller has no auto agent — they'll accept or reject your offer when they come back online.",
      showTimer: true,
    };
  }

  // Ports mpShowDealOutcome: checks the seller's agentConfig.active, then
  // either shows "pending" immediately or subscribes to the buyer's own
  // deal doc for up to 6s waiting for the agent to accept/reject.
  async function showDealOutcome(sellerUid: string, buyerUid: string, dealId: string) {
    try {
      ...
      const finish = (status: string) => {
        ...
        if (status === "accepted") setOutcome("accept");
        else if (status === "rejected") setOutcome("reject");
      ...
      fallbackTimer = setTimeout(() => finish("pending"), 6000);
    } catch (e) {
      console.error("[deal-outcome] error", e);
      setOutcome("pending");
    }
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const dealId = out.dealId as string;
      const sellerUid = listing.ownerId as string;
      ...
      // same 1400ms delay as the original.
      closeAfterSuccessRef.current = setTimeout(() => {
        onClose();
        showDealOutcome(sellerUid, buyerUid, dealId);
      }, 1400);
    } catch (e) {
          ...
          <div className={`mp-outcome-box ${outcomeContent.theme}`}>
            ...
            <div className="mp-outcome-icon-wrap">
              ...
              {outcome === "reject" && (
                <svg viewBox="0 0 52 52">
                  <circle className="mp-outcome-circle" cx="26" cy="26" r="24" fill="none" />
                  <path className="mp-outcome-path" fill="none" d="M18 18l16 16M34 18L18 34" />
                </svg>
              )}
              {outcome === "pending" && (
                <svg viewBox="0 0 52 52">
            ...
            <button className="mp-outcome-ok" onClick={closeOutcome}>
              Got it
            </button>
          </div>
        </div>
